...
class PoleServer_handler implements Runnable {
  ...
  */
  ...
  double posDot, double dest) {
    ...
    double action = 10 * angle + angleDot + posDot + move;
    System.out.println("initial action: " + action);
    // stabilize if the pendulum is rising
    // need to simplify this logic
    if (angle < 0 && posDot < 0)
      action += Math.abs(angleDot) * 2 + Math.abs(posDot);
    else if (angle > 0 && posDot > 0)
      action -= Math.abs(angleDot) * 2 + Math.abs(posDot);
    System.out.println("modified action: " + action);
    ...
    // ensure travel direction toward dest
    // if (pos < dest && angle < 0)
    //   action = -.2 + Math.abs(action) * -1;
    // else if (pos > dest && angle > 0)
    //   action =  .2 + Math.abs(action);
    // else if (pos < dest && angle > 0)
    //   action += 0;
    // else if (pos > dest && angle < 0)
    //   action -= 0;
    ...
    //
    // if ((angle < 0 && posDot > 0) || (angle > 0 && posDot < 0))
    //   action += posDot + angle * -1;
    // System.out.println("real action: " + action);
    return action;
  }
